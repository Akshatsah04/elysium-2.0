        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {gods.map((god, i) => (
            <motion.div
              ...
              className={`glass-card p-6 rounded-lg transition-all duration-500 cursor-pointer group ${colorStyles[god.color as keyof typeof colorStyles]}`}
            >
              <div className="flex items-start gap-4">
                <div className={`w-16 h-16 rounded-lg bg-muted flex items-center justify-center text-3xl font-display ${glowStyles[god.color as keyof typeof glowStyles]}`}>
                  {god.name.charAt(0)}
                </div>
                <div className="flex-1">
                  ...
                  <p className={`text-xs uppercase tracking-wider ${glowStyles[god.color as keyof typeof glowStyles]}`}>
                    {god.title}
                  </p>
